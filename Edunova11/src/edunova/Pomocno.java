...
public class Pomocno {
		...
		while(true) {
			...
			try {
				...
				if(vrati>=min && vrati<=max) {
					...
					System.err.println("Broj nije u rasponu " + min + " - " + max);
				}
			} catch (Exception e) {
				System.err.println("Nije cijeli broj u rasponu " + min + " - " + max);
			}
		}
	...
	public static String ucitajString(Scanner ulaz, String string) {
		...
		while(true) {
			...
			if(ucitao.trim().length()!=0) {
				...
				System.err.println("Obavezno unos niza znakova");
			}
		}
	}
	...
	public static BigDecimal ucitajDecimalniBroj(Scanner ulaz, String string) {
		...
		while(true) {
			...
			try {
				...
				System.err.println("Niste unijeli decimalni broj");
			}
		}
	...
	public static String slucajniString() {
        String SALTCHARS = "ABCDEFGHIJKLMNOPRSTUVZŽĆČŠ";
